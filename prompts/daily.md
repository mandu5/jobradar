...
Read profile/profile.md and profile/rubric.md fully. Do not spawn subagents, schedule wakeups, or end your turn before the final step — a delegated or deferred scoring never gets written. If egress is blocked, do not try WebFetch on job sites; grade from the fields present (title, company, career, deadline, location, snippet, tags) exactly as the rubric says.
...
Rules: never fabricate a posting or a link; every url must come from a collector, an email, or a page you opened. Do not apply to anything. Do not send email to anyone other than <YOUR EMAIL>. Finish with a short summary: counts (A/B/C), failed sources, and whether the tracker and email succeeded.
